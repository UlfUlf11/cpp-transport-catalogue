...
void ParseNodeSerialization(const Node& node, serialization::SerializationSettings& serialization_set)
{
    ...
    {
        std::cout << "serialization is not map";
    }
}


void ParseNodeProcessRequest(const Node& root, std::vector<StatRequest>& stat_request,
                             serialization::SerializationSettings& serialization_settings)
{
    ...
    if (root.IsMap())
    {
        ...
        ParseNodeSerialization(dict.at("serialization_settings"), serialization_settings);
...
void MakeCatalogue(const Node& root, TransportCatalogue& catalogue,
...
void InputMakeBase(std::istream& input, TransportCatalogue& catalogue, map_renderer::RenderSettings& render_settings, serialization::SerializationSettings& serialization_settings)
{
    //данные на ввод и вывод
    Document doc;
    doc = Load(input);

    MakeCatalogue(doc.GetRoot(), catalogue, render_settings, serialization_settings);
}
...
void InputProcessRequest(std::istream& input, std::vector<StatRequest>& stat_request, serialization::SerializationSettings& serialization_settings)
{
    ...
    ParseNodeProcessRequest(doc.GetRoot(), stat_request, serialization_settings);
}
