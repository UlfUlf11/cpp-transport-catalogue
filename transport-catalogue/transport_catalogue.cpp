...
void TransportCatalogue::AddDistance(const Stop* stop_a, const Stop* stop_b, int distance)
{
    ...
    // DistanceMap = std::unordered_map<std::pair<const Stop*, const Stop*>, int,
    // DistanceHasher>;
...
double TransportCatalogue::GetLength(Bus* bus)
{
    ...
    {
        return detail::geo::ComputeDistance(
        { (*lhs).coordinates.latitude, (*lhs).coordinates.longitude },
...
int TransportCatalogue::GetDistanceForStop(
    const Stop* point_a, const Stop* point_b)
{
    auto it = distance_beetween_stops_.find(std::make_pair(point_a, point_b));
    auto it2 = distance_beetween_stops_.find(std::make_pair(point_b, point_a));

    if (it != distance_beetween_stops_.end())
    {
        return distance_beetween_stops_.at(std::make_pair(point_a, point_b));
    }
    else if (it2 != distance_beetween_stops_.end())
    {
        return distance_beetween_stops_.at(std::make_pair(point_b, point_a));
    }
    else
    {
        return 0;
...
void TransportCatalogue::AddRouteSettings(
    const detail::RoutingSettings route_settings)
{
...
{
    return distance_beetween_stops_;
}

detail::RoutingSettings TransportCatalogue::GetRouteSettings() const
{
    RoutingSettings rs;
